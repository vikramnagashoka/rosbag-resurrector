...
// Notebook UI overhaul (v0.8) — now the DEFAULT surface at `/` (and `/n`).
// The classic dark pages moved behind `/classic/*`. Lazy so its scoped theme
// + Plotly cells don't load until visited.
...
// The classic page-per-feature layout (dark theme), now mounted under
// `/classic/*`. Child routes are RELATIVE to that mount point (v6 nested
// routing); classic links are absolute `/classic/...` (see NavBar + the
// bag-card links). The notebook workspace renders full-screen outside this.
    ...
    <ErrorToastProvider>
      <BrowserRouter>
        <Suspense fallback={<div style={loadingStyle}>Loading…</div>}>
          <Routes>
            {/* Notebook workspace is the DEFAULT surface (at `/` and `/n`) +
                its warm-themed workflow pages — full-screen, no NavBar/main
                chrome. Static paths rank above the :notebookId param, so
                /n/datasets wins over /n/:id. */}
            ...
            {/* The classic dark UI, namespaced behind /classic. */}
